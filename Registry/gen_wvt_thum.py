...
2026-09-18: SECTION 4 now wraps the region-2..N calls in the same `p_qv_tr` test region 1 has
(they were reachable only because check_a_mundo forbids num_wvt_regions>1 without tracer_opt=4
-- a guard by coincidence), and SECTION 6 emits Registry PACKAGE lines so region n's fields are
allocated/written only when num_wvt_regions >= n (see gen_wvt_cuten.py SECTION 9 for why).
"""
import sys
...
def main():
    ...
    print(" ENDIF")
    print("! wvt: regions 2..N are Registry-packaged on num_wvt_regions>=n (size-1 dummies below that),")
    print("! and the whole block is additionally gated on p_qv_tr: with tracers off nothing here may run.")
    print(" IF (p_qv_tr .ge. PARAM_FIRST_SCALAR) THEN")
    for n in regions:
        base = (n - 1) * 6
    ...
    print("==== SECTION 6: Registry.EM package lines (after the wvt_cuten_nr12 package line) ====")
    print("#wvt: tr_thum_{u,v}_phy_dt_0n likewise, generated by gen_wvt_thum.py.")
    for N in regions:
        names = ",".join(fld(c, n) for n in range(2, N + 1) for c in COMP)
        print(f"package   wvt_thum_nr{N:<4d}  num_wvt_regions=={N:<4d}   -             state:{names}")
    print()
