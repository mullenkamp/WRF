...
SECTION 9 (added 2026-09-18) emits Registry PACKAGE lines so that region n's members are
allocated -- and therefore written to history/restart -- only when num_wvt_regions >= n.
Cumulative `num_wvt_regions==N` packages listing regions 2..N, exactly like the tracer
members' wvt_nrN lines; several package lines may share a condition, only the package NAME
must be unique (each becomes an INTEGER PARAMETER in module_state_description), so this
generator owns its own `wvt_cuten_nrN` lines and never imports the others. Before this, every
_0N member was allocated and written as zeros in EVERY run, including tracer_opt=0
(~1.1 GB per production history frame). An inactive package field is a size-1 dummy: every
consumer of these fields is guarded (num_wvt_regions >= n and/or P_QV_TR), see the plan
record in wrf-model-eval. The registry lower-cases names, so case here is for readability.
"""
import sys
...
def main():
    ...
    print("==== SECTION 9: Registry.EM package lines (after the wvt_nr12 package line) ====")
    print("#wvt: RTRQ{V,C,I}CUTEN_0n are allocated (and so written to history/restart) only when")
    print("#     num_wvt_regions >= n: cumulative packages, one per N, generated by gen_wvt_cuten.py.")
    for N in regions:
        names = ",".join(f"RTRQ{sp.upper()}CUTEN_{n:02d}" for n in range(2, N + 1) for sp, *_ in SPECIES)
        print(f"package   wvt_cuten_nr{N:<4d} num_wvt_regions=={N:<4d}   -             state:{names}")
    print()
