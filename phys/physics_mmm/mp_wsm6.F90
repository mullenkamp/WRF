...
!
! --- determine if tracers are active ! wvt
 l_tracers = .false.                                               ! wvt
...
!=================================================================================================================
! wvt - tracer version of nislfv_rain_plm
!=================================================================================================================
 subroutine nislfv_rain_plm_tr(im,km,denl,denfacl,tkl,dzl,wwl,rql,precip,  &
                               tr_rql,tr_precip,dt,id,iter)
!=================================================================================================================
...
!--- inout arguments:
 real(kind=kind_phys),intent(inout),dimension(im):: precip,tr_precip
 real(kind=kind_phys),intent(inout),dimension(im,km):: rql,wwl,tr_rql
...
!---- local variables and arrays:
 ...
 real(kind=kind_phys):: th,th2,qqh,dqh
 real(kind=kind_phys):: zsum,qsum,dim,dip,c1,con1,fa1,fa2
 real(kind=kind_phys):: tr_qsum,tr_dim,tr_dip
 real(kind=kind_phys):: allold,allnew,zz,dzamin,cflmax,decfl
 real(kind=kind_phys),dimension(km):: dz,ww,qq,wd,wa,was
 real(kind=kind_phys),dimension(km):: den,denfac,tk
 real(kind=kind_phys),dimension(km):: qn,qr,tmp,tmp1,tmp2,tmp3
 real(kind=kind_phys),dimension(km):: tr_qn
 real(kind=kind_phys),dimension(km+1):: wi,zi,za
 real(kind=kind_phys),dimension(km+1):: dza,qa,qmi,qpi
 real(kind=kind_phys),dimension(km+1):: tr_qa,tr_qmi,tr_qpi
...
! computer deformation at arrival point
    do k=1,km
       qa(k) = qq(k)*dz(k)/dza(k)
       tr_qa(k) = tr_rql(i,k)*dz(k)/dza(k)                       ! wvt
       qr(k) = qa(k)/den(k)
    enddo
    qa(km+1) = 0.0
    tr_qa(km+1) = 0.0                                              ! wvt
...
! estimate values at arrival cell interface with monotone
    do k=2,km
       dip=(qa(k+1)-qa(k))/(dza(k+1)+dza(k))
       dim=(qa(k)-qa(k-1))/(dza(k-1)+dza(k))
       tr_dip=(tr_qa(k+1)-tr_qa(k))/(dza(k+1)+dza(k))            ! wvt
       tr_dim=(tr_qa(k)-tr_qa(k-1))/(dza(k-1)+dza(k))            ! wvt
       if( dip*dim.le.0.0 ) then
          qmi(k)=qa(k)
          qpi(k)=qa(k)
          tr_qmi(k)=tr_qa(k)                                      ! wvt
          tr_qpi(k)=tr_qa(k)                                      ! wvt
       else
          qpi(k)=qa(k)+0.5*(dip+dim)*dza(k)
          qmi(k)=2.0*qa(k)-qpi(k)
          tr_qpi(k)=tr_qa(k)+0.5*(tr_dip+tr_dim)*dza(k)          ! wvt
          tr_qmi(k)=2.0*tr_qa(k)-tr_qpi(k)                       ! wvt
          if( qpi(k).lt.0.0 .or. qmi(k).lt.0.0 ) then
             qpi(k) = qa(k)
             qmi(k) = qa(k)
             tr_qpi(k) = tr_qa(k)                                 ! wvt
    ...
    qmi(km+1)=qa(km+1)
    qpi(km+1)=qa(km+1)
    tr_qpi(1)=tr_qa(1)                                            ! wvt
    tr_qmi(1)=tr_qa(1)                                            ! wvt
    tr_qmi(km+1)=tr_qa(km+1)                                      ! wvt
    tr_qpi(km+1)=tr_qa(km+1)                                      ! wvt
!
! interpolation to regular point
    qn = 0.0
    tr_qn = 0.0                                                    ! wvt
...
! compute q with piecewise constant method
          if( kt.eq.kb ) then
             ...
             qql=qqd*tl2+qmi(kb)*tl
             qn(k) = (qqh-qql)/(th-tl)
!            --- tracer interpolation ! wvt
             qqd=0.5*(tr_qpi(kb)-tr_qmi(kb))                     ! wvt
             qqh=qqd*th2+tr_qmi(kb)*th                            ! wvt
             qql=qqd*tl2+tr_qmi(kb)*tl                            ! wvt
             tr_qn(k) = (qqh-qql)/(th-tl)                         ! wvt
          else if( kt.gt.kb ) then
             tl=(zi(k)-za(kb))/dza(kb)
             tl2=tl*tl
             qqd=0.5*(qpi(kb)-qmi(kb))
...
!            --- tracer interpolation ! wvt
             qqd=0.5*(tr_qpi(kb)-tr_qmi(kb))                     ! wvt
             qql=qqd*tl2+tr_qmi(kb)*tl                            ! wvt
             dql = tr_qa(kb)-qql                                   ! wvt
             tr_qsum = dql*dza(kb)                                 ! wvt
             if( kt-kb.gt.1 ) then
                do m=kb+1,kt-1
                   zsum = zsum + dza(m)
                   qsum = qsum + qa(m) * dza(m)
                   tr_qsum = tr_qsum + tr_qa(m) * dza(m)          ! wvt
                enddo
             endif
             th=(zi(k+1)-za(kt))/dza(kt)
             th2=th*th
             ...
             qn(k) = qsum/zsum
!            --- tracer interpolation ! wvt
             qqd=0.5*(tr_qpi(kt)-tr_qmi(kt))                     ! wvt
             dqh=qqd*th2+tr_qmi(kt)*th                            ! wvt
             tr_qsum = tr_qsum + dqh*dza(kt)                      ! wvt
             tr_qn(k) = tr_qsum/zsum                               ! wvt
          endif
          cycle intp
       endif
!
...
! rain out
    sum_precip: do k=1,km
       if( za(k).lt.0.0 .and. za(k+1).lt.0.0 ) then
          precip(i) = precip(i) + qa(k)*dza(k)
          tr_precip(i) = tr_precip(i) + tr_qa(k)*dza(k)           ! wvt
          cycle sum_precip
       else if ( za(k).lt.0.0 .and. za(k+1).ge.0.0 ) then
          precip(i) = precip(i) + qa(k)*(0.0-za(k))
          tr_precip(i) = tr_precip(i) + tr_qa(k)*(0.0-za(k))     ! wvt
          exit sum_precip
       endif
       exit sum_precip
    enddo sum_precip
...
! replace the new values
 ...
 end subroutine nislfv_rain_plm_tr

!=================================================================================================================
! wvt - tracer version of nislfv_rain_plm6
!=================================================================================================================
 subroutine nislfv_rain_plm6_tr(im,km,denl,denfacl,tkl,dzl,wwl,rql,rql2,  &
                                precip1,precip2,tr_rql,tr_rql2,             &
                                tr_precip1,tr_precip2,dt,id,iter)
!=================================================================================================================
...
!--- inout arguments:
 ...
 real(kind=kind_phys),intent(inout),dimension(im):: tr_precip1,tr_precip2
 real(kind=kind_phys),intent(inout),dimension(im,km):: rql,rql2,wwl
 real(kind=kind_phys),intent(inout),dimension(im,km):: tr_rql,tr_rql2
...
!---- local variables and arrays:
 ...
 real(kind=kind_phys),dimension(km+1):: tr_qa,tr_qa2,tr_qmi,tr_qpi
 real(kind=kind_phys),dimension(im):: precip
 real(kind=kind_phys),dimension(im):: tr_precip
...
! computer deformation at arrival point
    do k=1,km
       ...
       tr_qa(k) = tr_rql(i,k)*dz(k)/dza(k)                       ! wvt
       tr_qa2(k) = tr_rql2(i,k)*dz(k)/dza(k)                     ! wvt
       qr(k) = qa(k)/den(k)
       qr2(k) = qa2(k)/den(k)
    enddo
    qa(km+1) = 0.0
...
! replace the new values
       if(ist.eq.1) then
         ...
         precip1(i) = precip(i)
         tr_rql(i,:) = tr_qn(:)                                   ! wvt
         tr_precip1(i) = tr_precip(i)                              ! wvt
       else
         rql2(i,:) = qn(:)
         precip2(i) = precip(i)
         tr_rql2(i,:) = tr_qn(:)                                  ! wvt
         tr_precip2(i) = tr_precip(i)                              ! wvt
       endif
    enddo ist_loop

 enddo i_loop

 end subroutine nislfv_rain_plm6_tr
